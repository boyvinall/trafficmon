...
// clientInitial is RFC 9001 Appendix A.2's own worked "Client Initial"
// example — the same public test vector quicsni's own example_test.go uses —
// a complete QUIC v1 Initial packet carrying a ClientHello for "example.com".
...
// mustHexDecodeString strips the whitespace/newlines the RFC's own
// formatting of the test vector uses, then decodes it.
...
func TestQUICSNIInspectorInspect(t *testing.T) {
	inspector := NewQUICSNIInspector()

	t.Run("extracts SNI from a real Initial packet", func(t *testing.T) {
		host, ok := inspector.Inspect(clientInitial)
		if !ok || host != "example.com" {
			t.Fatalf("Inspect() = %q, %v; want %q, true", host, ok, "example.com")
		}
	})

	t.Run("not QUIC at all", func(t *testing.T) {
	...
	t.Run("truncated", func(t *testing.T) {
		if _, ok := inspector.Inspect(clientInitial[:128]); ok {
			t.Fatal("Inspect() = ok=true for an Initial packet truncated to 128 bytes")
		}
	})
}

func TestQUICSNIInspectorCandidate(t *testing.T) {
	tests := []struct {
		name string
		p    CandidatePacket
		want bool
